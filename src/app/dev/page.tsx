...
export default function DevDashboardPage() {
  ...
  const resetDefaults = async () => {
    if (!confirm('Reset orders back to pending and reopen products? This is developer-only.')) {
      return
    }
    ...
    <AdminLayout activeTab="developer">
      <header className="flex flex-col gap-6 border-b border-gray-100 pb-12">
        <div className="space-y-4">
          ...
          <p className="text-gray-400 text-[10px] uppercase tracking-[0.5em] font-medium">Trusted developer access for safe site maintenance.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            ...
            <ul className="space-y-3 text-sm leading-relaxed text-gray-600">
              ...
              <li>• Manage admin access securely by using the DEV_EMAIL setting.</li>
              <li>• Restore site defaults without changing the core brand experience.</li>
              <li>• Access both the admin dashboard and the developer route securely.</li>
            </ul>
          </div>
          <div className="space-y-4 bg-brand-cream/70 p-6 rounded-sm border border-brand-red/10">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">This developer portal is protected by the DEV_EMAIL environment variable and is only accessible to the configured developer account.</p>
