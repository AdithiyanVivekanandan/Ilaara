...
function Sidebar({ activeTab }: { activeTab: string }) {
  ...
  return (
    <aside className="w-full md:w-72 bg-white border-b border-gray-100 md:border-b-0 md:border-r flex min-h-screen flex-col justify-between relative md:fixed md:h-full z-20">
      <div>
        <div className="p-10">
      ...
      <nav className="flex-grow px-6 overflow-y-auto space-y-1">
        {navLinks.map((link) => (
          <Link 
            key={link.id}
            href={link.href} 
...
export default function AdminLayout({ children, activeTab }: { children: React.ReactNode, activeTab: string }) {
  return (
    <div className="min-h-screen bg-[#F9F8F6] flex flex-col md:flex-row font-sans selection:bg-brand-red selection:text-white">
      <Sidebar activeTab={activeTab} />
      <main className="flex-grow md:ml-72 p-6 md:p-16 space-y-16 relative z-10 transition-opacity duration-500 animate-in fade-in">
        {children}
      </main>
    </div>
  )
